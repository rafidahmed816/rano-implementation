...
def safe_normalize(wav: np.ndarray) -> np.ndarray:
    """Only scales down if the audio is clipping. Avoids amplifying noise floor."""
...
def calculate_mcd(
    ...
    wav_ref = safe_normalize(wav_ref)
    wav_deg = safe_normalize(wav_deg)
...
def evaluate(args):
    ...
    # FORCE GRIFFIN-LIM VOCODER AND 16kHz TO STOP PITCH SHIFTING
    ...
    with torch.no_grad():
        for path in tqdm(files, desc="Processing"):
            ...
            # Use raw vocoder output (no aggressive normalization)
            anon_wav = processor.mel_to_wav(xa.cpu()).squeeze().numpy()
            anon_wav = safe_normalize(anon_wav)

            # ── 2. Restore ───────────────────────────────────────────────────
            xr = rano_model.restore(xa, key)
            
            # Use raw vocoder output
            restored_wav = processor.mel_to_wav(xr.cpu()).squeeze().numpy()
            restored_wav = safe_normalize(restored_wav)
            ...
            base_name = path.stem
            sf.write(str(spk_out_dir / f"{base_name}_orig.wav"), wav_np, sr)
...
if __name__ == "__main__":
    ...
    # NEW ARGUMENT FOR SAVING AUDIO
    p.add_argument("--save_audio_dir",  type=str, default="eval_audio_outputs", 
